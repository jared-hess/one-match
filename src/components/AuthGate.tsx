...
export function AuthGate({ children }: AuthGateProps) {
    ...
    return (
      ...
      >
        <p className="rounded-3xl border border-blush-100 bg-cream-50/80 p-4 text-sm leading-6 text-ink-600">
          The route exists and renders safely without crashing. Add the public Vite Supabase URL and
          anon key to enable Google sign-in.
        </p>
      </PageShell>
    );
  }
  ...
  if (status === 'signed-out') {
    return (
      <PageShell
        eyebrow="Sign in"
        title="This DateJared room is private."
        description="Use Google sign-in to enter the MVP routes once Supabase auth is configured."
      >
        <button
          className="rounded-full bg-blush-500 px-5 py-3 text-sm font-bold text-cream-50 shadow-glow"
  ...
  return (
    <PageShell
      eyebrow="Loading"
      title="Preparing your DateJared room."
      description="Checking the local session before opening this route."
    />
  );
}
