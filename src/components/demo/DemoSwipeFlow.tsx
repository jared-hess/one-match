...
export function DemoSwipeFlow({
  ...
  const remainingLabel = useMemo(
    () =>
      `${Math.max(profiles.length - index, 0)} demo card${profiles.length - index === 1 ? '' : 's'} left`,
    ...
    <section className="space-y-5">
      <div className="rounded-app border border-white/80 bg-white/84 p-5 shadow-card backdrop-blur-xl">
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          <h1 className="font-display text-3xl font-semibold leading-none tracking-[-0.04em] text-merlot-900">
            Swipe locally.
          </h1>
          <span className="rounded-full bg-cream-50 px-3 py-1 text-xs font-extrabold text-ink-600">
            {remainingLabel}
          </span>
        </div>
      </div>
      ...
      <button
        className="w-full rounded-full border border-blush-100 bg-cream-50/80 px-5 py-3 text-sm font-extrabold text-merlot-900"
        onClick={onEndDemo}
        type="button"
      >
        End Demo
      ...
      {showSimilarity ? (
        <CandidateSimilarityModal onClose={() => setShowSimilarity(false)} />
      ) : null}
    </section>
  );
}
