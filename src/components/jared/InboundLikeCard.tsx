    ...
    <article className="rounded-app border border-white/80 bg-white/84 p-5 shadow-card backdrop-blur-xl">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-3xl border border-blush-100 bg-cream-50/80 p-4">
          <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-blush-600">Liked profiles</p>
          <p className="mt-2 text-2xl font-bold text-merlot-900">{likedProfiles.length}</p>
        </div>
        <div className="rounded-3xl border border-blush-100 bg-cream-50/80 p-4">
          <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-blush-600">Profile</p>
          <p className="mt-2 font-bold text-merlot-900">{isProfileCompleteForJared(userProfile) ? 'Complete' : 'Incomplete'}</p>
        </div>
      </div>
      <dl className="mt-5 space-y-2 text-sm leading-6 text-ink-600">
        ...
        <div>
          ...
          <dd>{notes.length} private note{notes.length === 1 ? '' : 's'}</dd>
        </div>
      </dl>
      <Link className="mt-5 inline-flex rounded-full bg-blush-500 px-5 py-3 text-sm font-extrabold text-cream-50 shadow-glow" to={`/jared/inbound/${relationship.id}`}>
        Review inbound like
      </Link>
    </article>
  );
