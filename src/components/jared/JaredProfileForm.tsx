...
export function JaredProfileForm({ profile, onSubmit }: JaredProfileFormProps) {
  ...
  function updateValue<Key extends keyof JaredProfileFormValues>(
    key: Key,
    value: JaredProfileFormValues[Key]
  ) {
  ...
  function updatePrompt(index: number, key: keyof PromptRow, value: string) {
    updateValue(
      'prompts',
      values.prompts.map((row, rowIndex) => (rowIndex === index ? { ...row, [key]: value } : row))
    );
  }
  ...
  function removeImageUrl(url: string) {
    updateValue(
      'image_urls',
      values.image_urls.filter((imageUrl) => imageUrl !== url)
    );
  }
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_24rem]">
      <form className="space-y-5" onSubmit={(event) => void handleSubmit(event)}>
        <section className="rounded-app border border-white/80 bg-white/84 p-5 shadow-card backdrop-blur-xl">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <Field label="Internal label">
              <input
                ...
                onChange={(event) => updateValue('internal_label', event.target.value)}
                required
                value={values.internal_label}
              />
            </Field>
            <Field label="Slug">
              <input
            ...
            <Field label="Age label">
              <input
                className={fieldClassName}
                onChange={(event) => updateValue('age_label', event.target.value)}
                value={values.age_label}
              />
            </Field>
          ...
          <Field label="Bio">
            <textarea
              className={`${fieldClassName} min-h-32 rounded-3xl`}
          ...
          <Field label="Tags">
            <input
              className={fieldClassName}
              onChange={(event) => syncTags(event.target.value)}
              placeholder="Dinner, Oakland, movies"
              value={tagText}
            />
          </Field>
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <Toggle
              checked={values.demo_eligible}
              label="Demo eligible"
              onChange={(checked) => updateValue('demo_eligible', checked)}
            />
          ...
          <div className="mt-5 space-y-4">
            {values.prompts.map((row, index) => (
              <div className="rounded-3xl border border-blush-100 bg-cream-50/80 p-4" key={index}>
                ...
                <Field label="Answer">
                  <textarea
                    className={`${fieldClassName} min-h-24 rounded-3xl`}
                    onChange={(event) => updatePrompt(index, 'answer', event.target.value)}
                    value={row.answer}
                  />
                </Field>
                <button
                  className="text-sm font-extrabold text-blush-600"
                  onClick={() =>
                    updateValue(
                      'prompts',
                      values.prompts.filter((_, rowIndex) => rowIndex !== index)
                    )
                  }
                  type="button"
                >
        ...
        <button
          className="w-full rounded-full bg-blush-500 px-5 py-3 text-sm font-extrabold text-cream-50 shadow-glow"
          disabled={saving}
          type="submit"
        >
...
function ProfilePreviewSurface({ profile }: { profile: JaredProfile }) {
  return (
    <section className="rounded-app border border-white/80 bg-white/84 p-4 shadow-card backdrop-blur-xl">
      <p className="mb-3 text-xs font-extrabold uppercase tracking-[0.24em] text-blush-600">
        Live normal-card preview
      </p>
...
const fieldClassName =
  'w-full rounded-full border border-blush-100 bg-cream-50/80 px-4 py-3 text-sm font-semibold text-ink-900 outline-none focus:border-blush-500';
