...
export function ChatThread({
  ...
  const canSend = Boolean(
    conversation &&
    currentUserId &&
    conversation.relationship.status === 'matched' &&
    ...
    <div className="space-y-4">
      <div className="rounded-app border border-blush-100 bg-cream-50/80 p-5">
        ...
        <div className="mt-3 flex items-center justify-between gap-3 text-sm leading-6 text-ink-600">
          <p>{statusText}</p>
          <button
            className="shrink-0 rounded-full border border-blush-100 bg-white/80 px-3 py-2 text-xs font-extrabold text-blush-600"
            onClick={onRefresh}
            type="button"
          >
            Refresh
      ...
      <div className="min-h-80 space-y-3 rounded-app border border-white/80 bg-white/70 p-4 shadow-card backdrop-blur-xl">
        {messages.length ? (
          messages.map((message) => {
            ...
            return (
              <MessageBubble
                ...
                message={message}
                own={own}
                senderLabel={own ? 'You' : otherName === 'Matched user' ? 'Jared' : otherName}
              />
            );
          })
        ) : (
          <div className="rounded-3xl border border-blush-100 bg-cream-50/80 p-5 text-sm leading-6 text-ink-600">
            ...
            <p className="mt-2">
              No messages yet. Pending, passed, unmatched, and archived relationships stay
              read-only.
            </p>
          </div>
        )}
      ...
      <MessageInput
        ...
        helperText={
          canSend
            ? 'Send a message only inside this matched, open Jared conversation.'
            : disabledReason
        }
        onSend={onSend}
      />
